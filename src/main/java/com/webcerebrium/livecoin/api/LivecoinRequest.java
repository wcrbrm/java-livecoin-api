...
public class LivecoinRequest {
    ...
    public LivecoinRequest(String requestUrl)  throws LivecoinApiException {

        this.requestUrl = requestUrl;
        try {
            url = new URL(requestUrl);
        } catch (MalformedURLException e) {
            throw new LivecoinApiException("Mailformed URL " + e.getMessage());
        }
     ...
     */
    public LivecoinRequest connect() throws LivecoinApiException {
        ...
        try {
            ...
            throw new LivecoinApiException("SSL Error " + e.getMessage() );
        } catch (KeyManagementException e) {
            throw new LivecoinApiException("Key Management Error " + e.getMessage() );
        }

        try {
            conn = (HttpsURLConnection)url.openConnection();
        } catch (IOException e) {
            throw new LivecoinApiException("HTTPS Connection error " + e.getMessage());
        }

        try {
            conn.setRequestMethod(method);
        } catch (ProtocolException e) {
            throw new LivecoinApiException("HTTP method error " + e.getMessage());
        }
     ...
     * @throws LivecoinApiException in case of any error
     */
    public LivecoinRequest read() throws LivecoinApiException {
        if (conn == null) {
            connect();
        }
        try {
            ...
            if (conn.getResponseCode() >= HttpURLConnection.HTTP_BAD_REQUEST) {
                ...
                if (obj.has("code") && obj.has("msg")) {
                    throw new LivecoinApiException("ERROR: " +
                            obj.get("code").getAsString() + ", " + obj.get("msg").getAsString() );
                }
            }
        } catch (IOException e) {
            throw new LivecoinApiException("Error in reading response " + e.getMessage());
        }
        return this;
    }

    public LivecoinRequest payload(JsonObject payload) {
